...
def train_multitask(args, load_model=False, unsupervised=False,supervised = False, epochs = 10):
    ...
    if load_model:
        saved = torch.load("supervised-3-epochs.pt", map_location=torch.device('cpu'))
        config = saved['model_config']

        model = MultitaskBERT(config)
        model_state_dict = saved['model']  # This contains the model parameters

        # Reshape fc_paraphrase.weight to match the current model's expectations
        if 'fc2.weight' in saved['model']:
            fc_paraphrase_weight = saved['model']['fc2.weight']
            # Assuming the current model expects [1, 1536], ensure the loaded weight matches this shape
            fc_paraphrase_weight_resized = fc_paraphrase_weight.view(1, -1)  # Reshape to [1, X]
            model_state_dict['fc_paraphrase.weight'] = fc_paraphrase_weight_resized

        # Adjust fc_paraphrase.bias if necessary
        if 'fc2.bias' in saved['model']:
            fc_paraphrase_bias = saved['model']['fc2.bias']
            # If the current model expects a bias of shape [1], and the loaded bias is not this shape, adjust accordingly
            if fc_paraphrase_bias.numel() != 1:
                fc_paraphrase_bias_resized = fc_paraphrase_bias[:1]  # Use only the first element
                model_state_dict['fc_paraphrase.bias'] = fc_paraphrase_bias_resized
...
if __name__ == "__main__":
    ...
    do_contrastive = False # Flag for contrastive learning
    if do_contrastive:
